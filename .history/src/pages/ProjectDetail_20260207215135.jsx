...
function ProjectDetail() {
  ...
  const montroResearchImages = useMemo(() => {
    if (slug !== "montro" || !project) return [];
    const researchSection = project.sections?.find(
      (sec) => sec.heading === "User Research"
    );
    return researchSection?.slides?.map((slide) => slide.src) || [];
  }, [slug, project]);

  const totalSlides = useMemo(() => {
    if (slug === "can-design") return carouselImages.length;
    if (slug === "posters") return posterImages.length;
    if (slug === "montro") return montroResearchImages.length;
    return 0;
  }, [slug, carouselImages, posterImages, montroResearchImages]);
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            ...
                            link.anchorId === "montro-research-carousel"
                          ) {
                            setIsModalOpen(true);
                            setCurrentSlide(0);
                          } else {
                      ...
                      <img
                        src={
                          slug === "can-design"
                            ...
                            : montroResearchImages[currentSlide] // [추가]
                        }
                        alt="Research Slide"
                        className="overlay-image-player"
                      />
                      {/* 화살표 및 인디케이터 로직은 기존과 동일 */}
                      {currentSlide > 0 && (
                        <button
